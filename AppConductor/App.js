...
export default function App() {
  ...
  //  Sincronizar los datos guardados en bloque al recuperar internet
  useEffect(() => {
    if (isOnline && db) {
      triggerBatchSync();
    }
  ...
  // Función para procesar y enviar la ubicación
  const processTelemetry = async (latitude, longitude, speedKmh) => {
  ...
  // Vaciar la base de datos local hacia el servidor
  const triggerBatchSync = async () => {
    if (!db || syncing) return;
    
    const rows = await db.getAllAsync('SELECT * FROM telemetry_queue ORDER BY id ASC;');
